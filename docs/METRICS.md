...
- **Formula**:
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://latex.codecogs.com/svg.latex?\color{white}\text{ATE}=\sqrt{\frac{1}{N}\sum_{i=1}^{N}\|\mathbf{p}_{est%2Ci}-\mathbf{p}_{gt%2Ci}\|^2}">
    <img alt="ATE Formula" src="https://latex.codecogs.com/svg.latex?\text{ATE}=\sqrt{\frac{1}{N}\sum_{i=1}^{N}\|\mathbf{p}_{est%2Ci}-\mathbf{p}_{gt%2Ci}\|^2}">
  </picture>
...
- **Formula**: $\displaystyle \text{Coverage} = \frac{|GT_{free} \cap Est_{known}|}{|GT_{free}|}$
...
- **Formula**: $\displaystyle \text{IoU} = \frac{|GT_{occ} \cap Est_{occ}|}{|GT_{occ} \cup Est_{occ}|}$
...
- **Formula**: $\displaystyle \text{SSIM}(x, y) = \frac{(2\mu_x\mu_y + C_1)(2\sigma_{xy} + C_2)}{(\mu_x^2 + \mu_y^2 + C_1)(\sigma_x^2 + \sigma_y^2 + C_2)}$
...
- **Algorithm**:
  ...
  $\displaystyle \text{Thickness} \approx 2 \times \text{mean}(D(p)) \quad \forall p \in S$
